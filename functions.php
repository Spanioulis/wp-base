...
function bones_ahoy()
{
  ...
  load_theme_textdomain('newmoon', get_template_directory() . '/library/translation');
...
function bones_register_sidebars()
{
  ...
  /*
	...
	register_sidebar(array(
		'id' => 'sidebar2',
		'name' => __( 'Sidebar 2', 'rodamon' ),
		'description' => __( 'The second (secondary) sidebar.', 'rodamon' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
    ...
    <article class="cf">
      <header class="comment-author vcard">
        ...
        */
        ...
        ?>
        <?php printf(__('<cite class="fn">%1$s</cite> %2$s', 'rodamon'), get_comment_author_link(), edit_comment_link(__('(Edit)', 'rodamon'), '  ', '')) ?>
        <time datetime="<?php echo comment_time('Y-m-j'); ?>"><a href="<?php echo htmlspecialchars(get_comment_link($comment->comment_ID)) ?>"><?php comment_time(__('F jS, Y', 'rodamon')); ?> </a></time>

      </header>
      <?php if ($comment->comment_approved == '0') : ?>
        <div class="alert alert-info">
          <p><?php _e('Your comment is awaiting moderation.', 'rodamon') ?></p>
        </div>
      <?php endif; ?>
...
function cambiar_nombre_entradas() {
    global $wp_post_types;
    $labels = &$wp_post_types['post']->labels;
    $labels->name = 'Feeds'; // Reemplaza 'NUEVO NOMBRE' por el nombre que desees utilizar
}
add_action( 'init', 'cambiar_nombre_entradas' );

function cambiar_nombre_entradas_menu() {
    global $menu;
